...
/*-- Simple helper for the creation of a temporary command buffer, use to record the commands to upload data, or transition images. -#1#
...
private:
  ...
  /*-- Callback function to catch validation errors  -#1#
  ...
  void initInstance()
  {
    ...
    /#1#/ This finds the KHR surface extensions needed to display on the right platform
    uint32_t     glfwExtensionCount = 0;
    const char** glfwExtensions     = glfwGetRequiredInstanceExtensions(&glfwExtensionCount);#1#
    ...
    m_instanceExtensions.insert(m_instanceExtensions.end(), sdl3Extensions, sdl3Extensions + sdl3ExtensionCount);
...
public:
  ...
  /*-- When leak are reported, set the ID of the leak here --#1#
...
public:
  void drawFrame()
  {
    ...
    endFrame(cmd);
    std::cout << downloadColorAttachment1()[10] << std::endl;
  }
private:
...
private:
  ...
  void recordComputeCommands(VkCommandBuffer cmd) const
  {
    ...
    const shaderio::PushConstantCompute pushValues{
        ...
        /*.rotationAngle = 1.2f * ImGui::GetIO().DeltaTime,  // Rotation speed adjusted with framerate#1#
  ...
  void recordGraphicCommands(VkCommandBuffer cmd)
  {
    ...
    utils::cmdTransitionImageLayout(cmd, m_gBuffer.getColorImage(1), VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    vkCmdBeginRendering(cmd, &renderingInfo);
    ...
    vkCmdDrawIndexed(cmd, 3, 1, 0, 0, 0);  // 3 indices, 1 instance, 0 index offset, 0 vertex offset
    ...
    vkCmdDrawIndexed(cmd, 3, 1, 3, 0, 0);  // 3 indices, 1 instance, 3 index offset, 0 vertex offset
    ...
    utils::cmdTransitionImageLayout(cmd, m_gBuffer.getColorImage(1), VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL);
  }
...
public:
  ...
  bool m_useImGui{false};  // Flag to control ImGui rendering
private:
  ...
  int32_t* downloadColorAttachment1()
  {
...
SDL_AppResult SDL_AppEvent(void* appstate, SDL_Event* event)
{
  Renderer::rendererEvent(event);
    ...
    case SDL_EVENT_WINDOW_RESIZED:
        {
          Renderer::m_windowResized = true; // just flag for resize
        }
